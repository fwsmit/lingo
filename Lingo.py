...
def draw_letter(root, row, column, color, text):
    ...
    border_color = tk.Frame(root, highlightbackground=BG_COLOR, highlightthickness=3, bd=0)
    c = tk.Canvas(
        root,
        width=160,
        height=160,
    ...
    if color == "Y":
        ...
        c.create_rectangle(3, 3, 159, 159,
                           fill=RED_COLOR if color == "R" else BG_COLOR,
                           outline=""
                           )
        c.create_text(80, 80, text=text, fill='white', font=font)
        border_color.grid(row=row, column=column)
...
def show_word(root, word, color_code, row):
    for i in range(len(color_code)):
        draw_letter(root, row, i, color_code[i], word[i].upper())
...
while True:
    ...
    turn = 1
    print("Answer:", word)
    show_word(app.root, [word[0], ".", ".", ".", ".", "."], ["R", "B", "B", "B", "B", "B"], turn)
    app.root.update()
    while True:
        while True:
            guess = input("Guess: ")
        ...
        show_word(app.root, guess, colors, turn)
        turn += 1
        if all(x == "R" for x in colors):
            print("WON WON WON WON WON WON WON")
            time.sleep(3)
            break
        w = "".join([word[i] if colors[i] == "R" else "." for i in range(len(guess))])
        show_word(app.root, w, color_code(w, word), turn)
        turn += 1
        app.root.update()
    clear(app.root)
